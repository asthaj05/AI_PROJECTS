...
class SafeTool:
    """
    Wrapper to prevent empty tool outputs from being sent to the model.
    agno does NOT sanitize tool outputs — OpenAI rejects empty messages.
    """
    ...
    async def arun(self, *args, **kwargs):
        try:
            ...
            return f"[{self.name} failed with error: {e}]"

        # 🔥 CRITICAL FIX: never allow empty output
        if result is None or not str(result).strip():
            return f"[{self.name} returned no data]"
...
duckduckgo = SafeTool(DuckDuckGoTools(), "DuckDuckGo")
wikipedia = SafeTool(WikipediaTools(), "Wikipedia")
...
async def run_mcp_travel_planner(destination: str, num_days: int, preferences: str, budget: int):
    ...
    try:
        ...
        mcp_tools = SafeTool(raw_mcp_tools, "MCP")
        ...
        travel_planner = Agent(
            ...
            tools=[mcp_tools, duckduckgo,wikipedia],
        ...
        **REQUIRED OUTPUT FORMAT:**
        ...
        if not response.content.strip():
            response.content = "No tool output available. Using default knowledge to generate itinerary."
...
if api_keys_provided:
    ...
    with col2:
        budget = st.number_input("Budget (INR)", min_value=100, max_value=10000, step=100, value=2000)
